<template>
  <div class="demo-list">
    <h4>基本用法</h4>
    <div>
      <yd-cell :showIcon="true" :isLink="true" @click.native="textToast1('我只传了文案一个参数')">
        <span slot="title">
          <label>只传文案</label>
        </span>
      </yd-cell>
      ...
      <yd-cell :showIcon="true" :isLink="true" @click.native="sucToast('操作成功')">
        <span slot="title">
          <label>Success</label>
        </span>
      </yd-cell>
      <yd-cell :showIcon="true" :isLink="true" @click.native="failToast('操作失败')">
        <span slot="title">
          <label>Fail</label>
        </span>
      </yd-cell>
      <yd-cell :showIcon="true" :isLink="true" @click.native="warnToast('操作警告')">
        <span slot="title">
          <label>Warn</label>
        </span>
      </yd-cell>
    ...
    <h4>加载提示</h4>
    <div>
      <yd-cell :showIcon="true" :isLink="true" @click.native="showLoading()">
        <span slot="title">
          <label>Loading</label>
        </span>
        <span slot="desc">带文案+带透明遮罩（默认）+自动消失</span>
      </yd-cell>
      <yd-cell :showIcon="true" :isLink="true" @click.native="showLoading2()">
        <span slot="title">
          <label>Loading</label>
        </span>
        <span slot="desc">不自动消失+不带遮罩</span>
      </yd-cell>
      <yd-cell :showIcon="true" :isLink="true" @click.native="hideLoading()">
        <span slot="title">
    ...
    <h4>自定义样式</h4>
    <div>
      <yd-cell :showIcon="true" :isLink="true" @click.native="cusBgToast('我修改了背景色和透明度')">
        <span slot="title">
      ...
      <yd-cell :showIcon="true" :isLink="true" @click.native="cusIconToast('自定义Icon')">
        <span slot="title">
          <label>自定义Icon</label>
        </span>
      </yd-cell>
    ...
    <div>
      <yd-cell :showIcon="true" :isLink="true" @click.native="idToast1('我设置了id为123')">
        <span slot="title">
          <label>我设置了id为123</label>
        </span>
      </yd-cell>
      <yd-cell :showIcon="true" :isLink="true" @click.native="idToast2('我设置了id为321')">
        <span slot="title">
